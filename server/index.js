...
import enforceHttpsOrWww from './middleware/enforceHttpsOrWww';
...
if (process.env.BUILD_FLAG_IS_DEV === 'false') {
  const enforceHttps = config('enforceHttps');
  const enforceWww = config('enforceWww');

  app.use(enforceHttpsOrWww({ https: enforceHttps, www: enforceWww }));
}
...
app.get('*', (request, response, next) => {
  ...
  try {
    const rendered = reactApplication(request, response);
