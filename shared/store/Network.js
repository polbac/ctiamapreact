...
export default class Network {
   ...
   * @return {Promise}
   */
  fetch = (url, { maxAge = Infinity, force = false, cacheKey, ...rest } = {}) => {
    const { history } = this;
    const actuallCacheKey = cacheKey || url;

    if (!history.has(actuallCacheKey)) {
      history.set(actuallCacheKey, {});
    }

    // Get reference point to history item
    const item = history.get(actuallCacheKey);
