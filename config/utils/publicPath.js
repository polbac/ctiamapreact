 ...
 */
export function getLocalApiUrl(args) {
  const { host, port, herokuAppName, passwordProtect } = args;

  // Used on Heroku PR apps
  if (herokuAppName) {
    return `https://${passwordProtect ? `${passwordProtect}@` : ''}${herokuAppName}.herokuapp.com/api`;
  }

  return `http://${passwordProtect ? `${passwordProtect}@` : ''}${host}:${port}/api`;
}
