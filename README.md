...
This project uses WordPress as a headless CMS, see [`ctia-wp`](https://github.com/CTIA-IT/ctia-wp).

All [WP API v2](https://developer.wordpress.org/rest-api/) requests are proxied through [`api/wordpress.js`](server/api/wordpress.js) via the [Wordpress store](shared/store/Wordpress.js).

Two config values are used to connect to the API:

* `WORDRPESSS_API_URL` for the default API
* `WORDRPESSS_CUSTOM_API_URL` for our custom additions

## Menus

Menus are defined in WordPress under Appearance > Menus and used to populate the menu defined in [`Header.js`](shared/containers/header/Header.js). Most pages link to pages in WP that will be fetched dynamically. Custom pages with custom URLs can also be created.

Since the out-of-the-box API from WP does not expose menus we have a custom endpoint for that.

We keep a copy of the top-level pages and functions to resolve links in [`utils/urlResolver.js`](shared/utils/urlResolver.js).

## Pages

Pages are in general resolved dynamically via the menu from WordPress via [a dynamic route](shared/MainApp.js#L48) that only allows for pages in our saved top-level categories. The slug is resolved and if it's found displayed by [Page](shared/routes/page/Page.js).

## Posts and categories

In a similar way, posts are resolved from WordPress were the post system is used with categories. Each news subpage is a different category. In this project we reference them by name/slug but for querying we need to translate to integer ids, mapping for this is in [`utils/wordpress.js`](shared/utils/wordpress.js).
